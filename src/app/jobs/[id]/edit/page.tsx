...
export default function EditJobPage() {
  ...
  // Form hook
  const { form, handleFormSubmit, isFormReady } = useJobEditForm({
    job,
    employerProfileId: employerProfile?.id,
    selectedSpecialties,
    onSubmit: (jobData: UpdateJobDto) => updateJobMutation.mutate(jobData),
  });
  ...
  // Initialize selected specialties when job data loads
  useEffect(() => {
    if (job?.specialties) {
      setSelectedSpecialties(job.specialties);
    }
  }, [job?.specialties, setSelectedSpecialties]);

  // Check if user owns this job
  useEffect(() => {
    if (job && employerProfile && job.employerProfileId !== employerProfile.id) {
      toast({
        title: 'Access Denied',
  ...
  }, [job, employerProfile, router, toast]);
  ...
  const handleSubmitWithStatus = (status: JobStatus) => {
    setValue('status', status);
    // Use setTimeout to ensure setValue is processed before form submission
    setTimeout(() => {
  ...
  }, [updateJobMutation.isSuccess, router]);
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8 max-w-4xl">
      ...
      <div className="mb-6 sm:mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-2">Edit Job Posting</h1>
        <p className="text-sm sm:text-base text-muted-foreground">
          Update your job posting to attract qualified medical professionals
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)}>
        ...
        <Card className="mb-6">
          ...
          <CardContent className="space-y-4">
            <div>
              <Label htmlFor="specialty" className="text-sm sm:text-base">Specialty (Optional)</Label>
              <div className="flex gap-2">
            ...
            {selectedSpecialties.length > 0 && (
              <div className="space-y-2">
                <Label className="text-sm font-medium">Selected Specialties ({selectedSpecialties.length})</Label>
                <div className="flex flex-wrap gap-2 p-3 bg-muted/30 rounded-md border">
                  {selectedSpecialties.map((specialty) => (
                    <Badge key={specialty.id} variant="secondary" className="gap-1 px-3 py-1">
                      {specialty.name}
                      <button
                        type="button"
          ...
          <CardFooter className="flex flex-col sm:flex-row justify-between gap-3 pt-6">
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
              <Button
                ...
                onClick={() => handleSubmitWithStatus(JobStatus.draft)}
                className="w-full sm:w-auto"
              >
              ...
              <Button
                type="button"
                disabled={isSubmitting || updateJobMutation.isPending}
                onClick={() => handleSubmitWithStatus(JobStatus.published)}
                className="w-full sm:w-auto"
              >
                {updateJobMutation.isPending ? (
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                ...
                <span className="hidden xs:inline sm:inline">Update & Publish</span>
