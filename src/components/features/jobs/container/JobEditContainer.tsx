...
interface JobEditContainerProps {
    jobId: string;
    initialJob?: Job | null;
}

export function JobEditContainer({ jobId, initialJob }: JobEditContainerProps) {
    const router = useRouter();
    const { data: job, isLoading, error } = useGetJob(jobId);

    // Use initialJob as a fallback or for faster loading if passed from SSR
    ...
    return <JobFormContainer mode="edit" existingJob={displayJob!} />;
}
