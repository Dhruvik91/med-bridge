...
export const attachmentService = {
  async findAll(ownerType?: string, ownerId?: string): Promise<Attachment[]> {
    let url = API_CONFIG.path.attachments;
  ...
  async findOne(id: string): Promise<Attachment> {
    const response = await httpService.get<Attachment>(`${API_CONFIG.path.attachments}/${id}`);
    return response.data;
  },

  async create(data: CreateAttachmentDto): Promise<Attachment> {
    const response = await httpService.post<Attachment>(API_CONFIG.path.attachments, data);
    return response.data;
  },

  async remove(id: string): Promise<void> {
    await httpService.delete(`${API_CONFIG.path.attachments}/${id}`);
  },
};
